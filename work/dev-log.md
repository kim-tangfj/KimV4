...
### 需求
- 模板编辑器中的变量（如 `{剧本内容}`）需要被实际替换
- 画幅参数需要被实际使用到 AI 生成的项目中
- 支持多个模板变量的替换

### 实现

#### 1. 新增 `buildPromptFromTemplate` 函数

**位置**: `src/utils/projectCreator.js`
...
| 文件 | 变更 |
|------|------|
| `src/utils/projectCreator.js` | +98 行 -6 行，新增 `buildPromptFromTemplate` 函数，修改 AI 调用逻辑 |

### Git 提交
- 提交：`8504941` - feat: template variable replacement
...
2. 系统读取激活的模板
   ↓
3. buildPromptFromTemplate() 替换变量
   - {剧本内容} → 用户输入的剧本
   - {画幅} → 用户选择的画幅 (16:9/9:16 等)
   - {时长} → 根据画幅自动设置 (9:16=8s, 其他=15s)
   ↓
4. 完整的提示词发送给 LLM
   ↓
5. LLM 返回结构化 JSON 数据
...
{
  "project": {
    "aspectRatio": "16:9"  // 系统自动设置
  },
  "shots": [
    {
      "aspectRatio": "16:9"  // 每个片段都会设置
    }
  ]
}
```
...
### 工作内容
使用 agent 深入分析项目实际代码结构，更新 TODO 清单和开发计划。
...
| 文件 | 行数 | 功能 |
|------|------|------|
| `src/main.js` | ~300 | Electron 主进程、自动更新、加密存储 |
| `src/preload.js` | ~150 | IPC 桥接、安全 API 暴露 |

#### IPC 处理器（handlers/）
| 文件 | 行数 | 功能 |
|------|------|------|
| `project.js` | 1048 | 项目 CRUD、素材管理、分镜图片 |
| `api.js` | ~150 | 4 家 LLM API 调用 |
| `template.js` | ~150 | 模板加载/保存/备份 |
| `options.js` | ~350 | 自定义选项管理 |

#### 工具模块（utils/）- 18 个文件
| 文件 | 行数 | 功能 |
|------|------|------|
| `appStateManager.js` | ~350 | 统一状态管理 |
| `promptGenerator.js` | 339 | 提示词生成 |
| `menu.js` | ~300 | 原生菜单、日志窗口 |
| `userDataManager.js` | ~200 | 用户数据路径管理 |
| `errorLogger.js` | ~150 | 错误日志记录 |
| `ipcErrorHandler.js` | ~100 | IPC 错误处理 |
| `projectAssets.js` | - | 项目素材管理 |
| `sceneAssets.js` | - | 场景素材管理 |
| `propertyPanel.js` | - | 属性面板管理 |
| `settings.js` | - | 设置管理 |
| `customOptions.js` | - | 自定义选项 UI |
| `eventListeners.js` | - | 事件监听器 |
| `projectList.js` | - | 项目列表 |
| `shotList.js` | - | 片段列表 |
| `sceneList.js` | - | 镜头列表 |
| `templateLibrary.js` | - | 模板库 |
| `projectCreator.js` | - | 项目创建 |
| `uiHelpers.js` | - | UI 工具函数 |
...
- 项目管理（创建/打开/保存/删除/迁移）
- 分镜管理（片段/镜头增删改、分镜图片）
- 素材管理（项目/片段素材库、预览/删除/导出）
- 提示词生成（4 家 LLM、5 个模板）
- 设置管理（双主题、API Key 加密、恢复出厂）
- 系统功能（打包、自动更新、日志）

✅ **自定义选项 100% 完成**
- 风格 50+、情绪氛围 30+、配乐风格 25+、音效 30+
- 景别/角度/运镜 35+
- 总计约 150 个选项
- 内置选项 + 自定义选项 + 使用统计
...
| 功能 | 工时 | 说明 |
|------|------|------|
| 片段批量操作 | 4h | 批量删除/复制/移动 |
| 镜头批量操作 | 4h | 批量删除/复制/移动 |
| 项目批量导入/导出 | 6h | JSON 批量处理 |
| 快捷键支持 | 4h | Ctrl+N/S/Z 等 |

#### 第三阶段：模板增强（1-2 周）
| 功能 | 工时 | 说明 |
|------|------|------|
| 模板可视化编辑器 | 12h | 所见即所得编辑 |
| 模板导入/导出 | 4h | 分享和备份 |
| 提示词历史记录 | 4h | 记录生成历史 |
...
| 功能 | 工时 | 说明 |
|------|------|------|
| 分镜时间轴视图 | 16h | 可视化时间轴编辑 |
| AI 智能推荐 | 12h | 推荐景别/角度/运镜 |
| 多项目对比 | 8h | 并排对比视图 |
...
| 问题 | 优先级 | 建议 |
|------|--------|------|
| renderer.js 873 行 | 中 | 继续拆分 list 相关模块 |
| 部分函数缺少 JSDoc | 低 | 补充注释 |
| 无自动化测试 | 中 | 添加关键功能单元测试 |
| Markdown 渲染简陋 | 低 | 使用成熟库 |

### 版本规划

| 版本 | 计划日期 | 核心功能 |
|------|----------|----------|
| v1.0.4 | 2026-03-17 | 修复拼写、镜头素材库、素材统计 |
| v1.1.0 | 2026-03-24 | 批量操作、快捷键、导入导出 |
| v1.2.0 | 2026-03-31 | 模板编辑器、历史记录 |
| v1.3.0 | 2026-04-14 | 时间轴视图、AI 推荐 |

### 修改文件
| 文件 | 变更说明 |
|------|----------|
| `work/dev-log.md` | 更新本条目 |

---

## 2026-03-10 - 项目 TODO 整理与推荐计划

### 工作内容
阅读开发规范 (rules.md) 和项目摘要 (PROJECT_SUMMARY.md)，整理项目完整的 TODO 清单和推荐开发计划。

### 项目状态
- **当前版本**: v1.0.3
- **最近提交**: d36d331 project_summary_03-10
- **Git 状态**: 工作区干净，分支最新
...
| 模块 | 核心功能 |
|------|----------|
| 项目管理 | 创建/打开/删除、数据本地存储、用户数据目录迁移 |
| 分镜管理 | 片段/镜头管理、属性编辑、分镜图片上传/拖放 |
| 素材管理 | 项目/片段素材库、预览/删除、拖放上传、引用检查 |
| 提示词生成 | 4 家 LLM 集成、模板系统、提示词预览/复制/导出 |
| 设置管理 | 双主题、API Key 加密存储、恢复出厂设置 |
| 系统功能 | Electron 打包、自动更新、更新进度模态框 |

### 待修复问题

| 优先级 | 问题 | 模块 |
|--------|------|------|
| P0 | 镜头级别选项同步未验证 | 自定义选项 |
| P1 | 大文件 (>50MB) 上传较慢 | 素材管理 |
| P2 | 片段素材库缺少镜头素材引用检查 | 素材管理 |

### 待实现功能（优先级排序）

| 优先级 | 功能 | 模块 | 预计工时 |
|--------|------|------|----------|
| P0 | 批量导入/导出项目 | 项目管理 | 6h |
| P0 | 片段/镜头批量操作 | 分镜管理 | 8h |
| P1 | 镜头素材库 | 素材管理 | 4h |
| P1 | 素材使用情况统计 | 素材管理 | 2h |
| P1 | 模板可视化编辑器 | 模板系统 | 12h |
| P1 | 快捷键支持 | 全局 | 4h |
| P2 | 分镜时间轴视图 | 分镜管理 | 16h |
| P2 | 提示词历史记录 | 提示词生成 | 4h |
...
1. **renderer.js 文件过大** (3300+ 行) → 拆分为独立模块
2. **部分函数缺少注释** → 添加 JSDoc 注释
3. **部分 IPC 调用缺少错误处理** → 统一错误处理机制
4. **无自动化测试** → 添加关键功能单元测试

### 修改文件
| 文件 | 变更说明 |
|------|----------|
| `work/dev-log.md` | 新增本条目 |
